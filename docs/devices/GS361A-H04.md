...
### Installing the Thermostatic Radiator Valve (TRV)
- open battery cover (twist and slide)
- insert batteries or remove tab from batteries (an orange wrench should appear on the display)
- close battery cover (slide and twist)
- press front button until `F1₁` is displayed
- mount TRV on radiator
- press front button until `F1₂` is displayed (after 10-30 seconds the TRV should be able to pair)
- to return to the menu on an already installed TRV - long-press the front button until `F!` is shown (ignore lock symbol appear and disappear). Then turn right until `F4` appears and confirm by short-pressing the front button
